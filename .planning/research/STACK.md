...
**Why NOT web3.py:** py-clob-client already handles blockchain interaction internally. Adding web3.py would be redundant unless you need direct contract calls beyond Polymarket's CLOB API.

**Why NOT viem:** Viem is TypeScript-only. For Python stack, ethers.py is the correct choice.
...
**Why NOT aiohttp:** Overkill for Polymarket's API scale. AIOHTTP wins at >300 concurrent requests, but Polymarket scanning typically involves 50-100 markets. HTTPX's HTTP/2 support provides better real-world latency for API polling.
...
**Why NOT socket.io:** Polymarket uses standard WebSocket protocol, not Socket.IO. Adding Socket.IO would add unnecessary complexity.
...
**Why NOT InfluxDB/QuestDB:** Overkill for this scale. InfluxDB 3 Core has 72h retention limit. QuestDB is for HFT systems processing millions of rows/sec. SQLite handles 100K+ writes/day easily, which is sufficient for arbitrage trade logs.
...
**Why NOT structlog:** Structlog requires significant configuration overhead. Loguru works out-of-the-box and provides structured logging via `serialize=True`. Structlog is better for teams with strict enterprise logging standards — overkill for a solo trading bot.
...
**Alternative:** Direct HTTP calls via httpx for simple notifications (no library needed if just posting to webhook).
...
**Why NOT PM2:** PM2 is Node.js-only. This bot uses Python.

**Why NOT systemd directly:** Docker provides better isolation, easier updates (pull new image + restart), and portable configuration across VPS providers.
...
```bash
# Core dependencies
pip install py-clob-client ethers python-dotenv

# HTTP + WebSocket
pip install httpx websockets

# Logging + Notifications
pip install loguru python-telegram-bot discord.py
